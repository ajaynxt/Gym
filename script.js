...
let previewState = (() => {
  ...
  return {
    ...
    displayMode: staticConfig.displayMode === 'light' || staticConfig.displayMode === 'dark'
      ? staticConfig.displayMode
      : (saved.displayMode === 'light' || saved.displayMode === 'dark'
        ? saved.displayMode
...
function applyHue(hue) {
  ...
  const accent = hslToRgb(hue, 92, previewState.displayMode === 'light' ? 42 : 48);
  const accent2 = hslToRgb(hue2, 92, previewState.displayMode === 'light' ? 48 : 58);
  const luminance = (0.2126 * accent[0] + 0.7152 * accent[1] + 0.0722 * accent[2]) / 255;
  const style = document.documentElement.style;
  style.setProperty('--orange', `hsl(${hue} 92% ${previewState.displayMode === 'light' ? 42 : 48}%)`);
  style.setProperty('--orange2', `hsl(${hue2} 92% ${previewState.displayMode === 'light' ? 48 : 58}%)`);
  ...
  if (previewState.displayMode === 'light') {
    style.setProperty('--bg', `hsl(${hue} 28% 98%)`);
    style.setProperty('--bg2', `hsl(${hue} 22% 95%)`);
    style.setProperty('--card', '#ffffff');
    style.setProperty('--text', `hsl(${hue} 24% 10%)`);
    style.setProperty('--muted', `hsl(${hue} 10% 38%)`);
    style.setProperty('--border', `hsl(${hue} 16% 84%)`);
    style.setProperty('--navy', `hsl(${hue} 38% 18%)`);
    style.setProperty('--shadow', `0 18px 55px hsla(${hue},30%,20%,.14)`);
  } else {
    style.setProperty('--bg', `hsl(${hue} 18% 4%)`);
    style.setProperty('--bg2', `hsl(${hue} 16% 7%)`);
    style.setProperty('--card', `hsl(${hue} 13% 10%)`);
    style.setProperty('--text', '#f7f7f8');
    style.setProperty('--muted', '#a6a6ad');
    style.setProperty('--border', `hsl(${hue} 11% 20%)`);
    style.setProperty('--navy', `hsl(${hue} 36% 15%)`);
    style.setProperty('--shadow', '0 18px 55px rgba(0,0,0,.32)');
  }
...
function refreshModeButton() {
  ...
  if (!button) return;
  const nextMode = previewState.displayMode === 'dark' ? 'light' : 'dark';
  button.innerHTML = `${modeIcon(previewState.displayMode)}<span>${previewState.displayMode === 'dark' ? 'Dark' : 'Light'}</span>`;
  button.setAttribute('aria-label', `Switch to ${nextMode} mode`);
  button.setAttribute('title', `Switch to ${nextMode} mode`);
  button.setAttribute('aria-pressed', String(previewState.displayMode === 'light'));
}

function applyDisplayMode(mode, announce = false) {
  previewState.displayMode = mode === 'light' ? 'light' : 'dark';
  document.documentElement.dataset.mode = previewState.displayMode;
  document.documentElement.style.colorScheme = previewState.displayMode;
  applyHue(previewState.hue);
  refreshModeButton();
  savePreview();
  if (announce) showToast(`${previewState.displayMode === 'light' ? 'Light' : 'Dark'} mode active`);
}
...
function installModeToggle() {
  ...
  button.addEventListener('click', () => {
    applyDisplayMode(previewState.displayMode === 'dark' ? 'light' : 'dark', true);
  });
  const navWrap = document.querySelector('.nav-wrap');
  const navCta = document.querySelector('.nav-cta');
  if (navWrap) navWrap.insertBefore(button, navCta || null);
  refreshModeButton();
}
...
installModeToggle();
applyDisplayMode(previewState.displayMode);
