...
// ---------- AJAYNXT AUDIT UPDATE: HERO DETAILS + FONT/MODE + AUTO PREVIEW ----------
const heroDetailsModal = document.getElementById('hero-details-modal');
const heroDetailsOpen = document.getElementById('hero-details-open');
function setHeroDetails(open){
  if(!heroDetailsModal) return;
  heroDetailsModal.classList.toggle('open',open);
  heroDetailsModal.setAttribute('aria-hidden',String(!open));
  document.body.classList.toggle('hero-modal-open',open);
  if(open) heroDetailsModal.querySelector('.hero-details-close')?.focus();
  else heroDetailsOpen?.focus();
}
heroDetailsOpen?.addEventListener('click',()=>setHeroDetails(true));
document.querySelectorAll('[data-hero-details-close]').forEach(btn=>btn.addEventListener('click',()=>setHeroDetails(false)));
document.querySelectorAll('a[href="#free-trial"]').forEach(link=>link.addEventListener('click',event=>{event.preventDefault();setHeroDetails(true);}));
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&heroDetailsModal?.classList.contains('open'))setHeroDetails(false);});

const EXTRA_PREVIEW_KEY='ironpulse-extra-preview-v1';
const extraSaved=JSON.parse(localStorage.getItem(EXTRA_PREVIEW_KEY)||'{}');
let extraPreview={font:extraSaved.font||'barlow',lockFont:Boolean(extraSaved.lockFont),mode:extraSaved.mode||'dark',auto:extraSaved.auto!==false};
const fontChoices=['barlow','inter','serif'];
const themeChoices=Object.keys(themeSets);
const copyChoices=Object.keys(copySets);
function saveExtraPreview(){localStorage.setItem(EXTRA_PREVIEW_KEY,JSON.stringify(extraPreview));}
function applyFont(name){
  extraPreview.font=fontChoices.includes(name)?name:'barlow';
  document.body.dataset.font=extraPreview.font;
  document.querySelectorAll('[data-font-choice]').forEach(btn=>btn.classList.toggle('active',btn.dataset.fontChoice===extraPreview.font));
}
function applyMode(mode){
  extraPreview.mode=mode==='light'?'light':'dark';
  document.body.dataset.mode=extraPreview.mode;
  const button=document.getElementById('mode-toggle');
  if(button) button.textContent=extraPreview.mode==='dark'?'Light Mode':'Dark Mode';
}
function refreshExtraControls(){
  document.querySelectorAll('[data-font-choice]').forEach(btn=>btn.disabled=extraPreview.lockFont);
  const label=document.getElementById('font-lock-label'); if(label) label.textContent=extraPreview.lockFont?'Locked':'Unlocked';
  const lock=document.getElementById('lock-font'); if(lock){lock.textContent=extraPreview.lockFont?'Font Locked':'Lock Font';lock.classList.toggle('locked',extraPreview.lockFont);}
  const auto=document.getElementById('auto-preview'); if(auto) auto.textContent=extraPreview.auto?'Pause Auto • 60s':'Play Auto • 60s';
}
function updateExtraPreview(){applyFont(extraPreview.font);applyMode(extraPreview.mode);refreshExtraControls();saveExtraPreview();}
document.querySelectorAll('[data-font-choice]').forEach(btn=>btn.addEventListener('click',()=>{if(extraPreview.lockFont)return;applyFont(btn.dataset.fontChoice);saveExtraPreview();showToast(`${btn.querySelector('b').textContent} font applied`);}));
document.getElementById('lock-font')?.addEventListener('click',()=>{extraPreview.lockFont=true;updateExtraPreview();showToast('Font choice locked');});
document.getElementById('mode-toggle')?.addEventListener('click',()=>{applyMode(extraPreview.mode==='dark'?'light':'dark');saveExtraPreview();});
document.getElementById('auto-preview')?.addEventListener('click',()=>{extraPreview.auto=!extraPreview.auto;refreshExtraControls();saveExtraPreview();showToast(extraPreview.auto?'Auto preview started':'Auto preview paused');});
document.getElementById('lock-both')?.addEventListener('click',()=>{extraPreview.lockFont=true;updateExtraPreview();});
document.getElementById('unlock-all')?.addEventListener('click',()=>{extraPreview={font:'barlow',lockFont:false,mode:'dark',auto:true};updateExtraPreview();});
function nextChoice(list,current){const index=list.indexOf(current);return list[(index+1+list.length)%list.length];}
setInterval(()=>{
  if(!extraPreview.auto)return;
  let changed=false;
  if(!previewState.lockTheme){previewState.theme=nextChoice(themeChoices,previewState.theme);changed=true;}
  if(!previewState.lockCopy){previewState.copy=nextChoice(copyChoices,previewState.copy);changed=true;}
  if(!extraPreview.lockFont){extraPreview.font=nextChoice(fontChoices,extraPreview.font);changed=true;}
  if(changed){updatePreview();updateExtraPreview();showToast('Auto preview changed color, text and font');}
},60000);
updateExtraPreview();
